...
int MD5_16(const char* const pMsg,size_t msg_len, char pOut[])
{
	assert(NULL != pMsg && NULL != pOut);
	MD5_CTX	ctx;
	MD5Init(&ctx);

	MD5Update(&ctx,(unsigned char *)pMsg,(unsigned int)msg_len);

	MD5Final((unsigned char*)pOut, &ctx);
...
int MD5_32(const char* const data, size_t data_len, char out_put[32])
{
	...
	MD5Final(hash, &ctx);
	bin2hex(out_put, hash, sizeof(hash));
	return 0;
}
